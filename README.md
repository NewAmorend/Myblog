...
后台左侧可在“文章 / 系列”之间切换。文章编辑器通过“内容归属”选择独立文章或系列章节；系列编辑器负责简介、阅读前提、章节加入/移出与顺序调整。系列目录发布后会直接更新 `series/index.json` 和 sitemap。
...
日常发布优先访问 `/admin/` 使用写作后台。后台支持草稿、Markdown 分栏预览、图片上传、独立文章与系列章节归属、系列目录排序、发布更新和下线。本机草稿保存在 SQLite，单独部署的线上后台使用私有 Redis；发布、下线、系列目录变更和图片上传才会形成 Git 提交，因此不改变现有静态前台的数据结构。
