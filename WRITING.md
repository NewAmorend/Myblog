...
1. 点击“新建文章”，填写标题、标签、分类和摘要；在“内容归属”中选择“独立文章”或已有系列。
...
1. 先创建系列，填写标题、简介和可选的阅读前提。
2. 发布文章时选择这个系列，文章会自动成为最后一章；也可以在系列编辑器里从已发布文章中加入章节。
3. 用上下按钮调整阅读顺序。系列顺序与文章发布日期无关。
4. 点击“发布系列”后，后台一次性更新 `series/index.json` 与 sitemap。下线系列只删除目录，章节正文会回到独立文章列表。
...
- 后台可直接创建、下线和排序系列，也能在文章编辑器中设置章节归属。已经存在的系列翻译字段会在后台更新时保留；新增或修改译文仍需直接维护 `series/index.json`。
